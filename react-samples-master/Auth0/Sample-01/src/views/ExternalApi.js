...
export const ExternalApiComponent = () => {
  ...
  const tokenFieldLibrary = {
    iss: {
      title: "Issuer (iss)",
      description: "Identifica il Security Token Service che ha emesso il JWT. Deve coincidere con il dominio Auth0 atteso: se non combacia, il token va rifiutato per evitare spoofing.",
    },
    sub: {
      title: "Subject (sub)",
      description: "ID univoco dell'utente presso l'issuer. E stabile nel tempo e va usato come chiave tecnica per collegare profilo e autorizzazioni.",
    },
    aud: {
      title: "Audience (aud)",
      description: "Lista dei destinatari previsti. La tua API deve essere presente in aud, altrimenti il token non e valido per quella risorsa.",
    },
    exp: {
      title: "Expires (exp)",
      description: "Istante di scadenza in epoch seconds. Dopo questo momento il token deve essere rifiutato per ridurre la finestra di rischio.",
    },
    iat: {
      title: "Issued At (iat)",
      description: "Momento di emissione del token. Serve per audit, debugging e per capire l'eta del token durante la validazione.",
    },
    nbf: {
      title: "Not Before (nbf)",
      description: "Il token e valido solo dopo questo istante. Utile per prevenire utilizzi anticipati o clock skew non gestito.",
    },
    scope: {
      title: "Scope",
      description: "Permessi richiesti dal client (es. read:orders). L'API deve verificarli e concedere solo le azioni permesse.",
    },
    permissions: {
      title: "Permissions",
      description: "Permessi RBAC calcolati da Auth0 per questa audience. Sono ideali per i check di autorizzazione a livello API.",
    },
    azp: {
      title: "Authorized Party (azp)",
      description: "Client che ha ottenuto il token. Consente di tracciare l'app chiamante in scenari multi-client.",
    },
    kid: {
      title: "Key Id (kid)",
      description: "Identificatore della chiave di firma. L'API recupera il JWKS e usa il kid per scegliere la chiave corretta.",
    },
    alg: {
      title: "Algorithm (alg)",
      description: "Algoritmo di firma (es. RS256). L'API deve accettare solo algoritmi consentiti per evitare downgrade.",
    },
    typ: {
      title: "Type (typ)",
      description: "Tipo di token (tipicamente JWT). Aiuta i consumer a interpretare correttamente il contenuto.",
    },
    jti: {
      title: "JWT ID (jti)",
      description: "Identificatore univoco del token. Utile per prevenire replay tramite allow/deny list.",
    },
    auth_time: {
      title: "Auth Time",
      description: "Momento dell'ultima autenticazione interattiva. Utile per MFA e step-up con requisiti di freschezza.",
    },
    client_id: {
      title: "Client ID",
      description: "Identificatore dell'applicazione che ha richiesto il token (client). Utile per auditing e policy specifiche per client.",
    },
    email: {
      title: "Email",
      description: "Email dell'utente quando richiesta nello scope. Non usarla come chiave unica: preferisci sub.",
    },
    name: {
      title: "Name",
      description: "Nome visualizzato dell'utente. Informazione di profilo, non adatta come identificatore.",
    },
    roles: {
      title: "Roles",
      description: "Ruoli applicati all'utente secondo il modello RBAC. Usali insieme ai permissions per autorizzare.",
    },
    "CIAM DEMO/roles": {
      title: "CIAM DEMO roles",
      description: "Indica i ruoli dell'utente assegnati nel contesto CIAM DEMO.",
    },
    "https://auth.rocks/email": {
      title: "Auth.rocks email",
      description: "Indica l'email dell'utente nel namespace custom auth.rocks.",
    },
    "https://auth.rocks/name": {
      title: "Auth.rocks name",
      description: "Indica il nome dell'utente nel namespace custom auth.rocks.",
    },
  };
      ...
      {Object.entries(data || {}).map(([key, value]) => {
        const meta =
          tokenFieldLibrary[key] || {
            title: key,
            description: "Claim personalizzato aggiunto da API, Rule o Action. Documentalo e validalo lato API per evitare ambiguita.",
          };
        return (
          <div
